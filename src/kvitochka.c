...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/27 15:42:15 by ikachko           #+#    #+#             */
/*   Updated: 2018/05/27 15:42:15 by ikachko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_kvitochka(t_window *win)
{
	...
	{
		win->tmp = win->z_re;
		win->z_re = ((win->tmp * win->tmp) -
					(win->z_im * win->z_im) - win->tmp) + win->c_re;
		win->z_im = 2 * (win->tmp * win->z_im) - win->z_im + win->c_im;
		win->it++;
	}
	if (win->it == win->it_max)
		put_pxl_to_img(win, win->y, WIN_WIDTH - win->x - 1, 0xFFFFFF);
	else
		put_pxl_to_img(win, win->y, WIN_WIDTH - win->x - 1,
												win->color * win->it);
}
...
void	kvitochka_pthread(t_window *win)
{
	...
	i = 0;
	while (++i <= 10)
		pthread_create(&t[i - 1], NULL, kvitochka, &tab[i - 1]);
	while (i--)
