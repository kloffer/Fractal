...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2018/05/27 15:41:10 by ikachko           #+#    #+#             */
/*   Updated: 2018/05/27 15:41:11 by ikachko          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	calc_bob(t_window *win)
{
	...
	{
		win->tmp = win->z_re;
		win->z_re = (win->z_re * win->z_re
					- win->z_im * win->z_im) + win->c_re * win->c_re;
		win->z_im = (2 * win->z_im * win->tmp) + win->c_im * win->c_im;
		win->it++;
	}
...
void	bob_pthread(t_window *win)
{
	...
	i = 0;
	while (++i <= 10)
		pthread_create(&t[i - 1], NULL, bob, &tab[i - 1]);
	while (i--)
